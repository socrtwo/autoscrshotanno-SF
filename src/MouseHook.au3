...
$DidUserClick = False  ; Value to determine if mouse has been right clicked
...
Global $hhCBT = DLLCall("user32.dll","hwnd","SetWindowsHookEx","int",5, _
	"hwnd",$cbtHOOKproc[0],"hwnd",$DLLinst[0],"int",0)

;DLLCall(".\hook.dll","int","SetValuesMouse","hwnd",$gui,"hwnd",$hhMouse[0])
DLLCall(".\hook.dll","int","SetValuesMouse","hwnd",$gui,"hwnd",7)
...
GUIRegisterMsg($WM_AUTOITRBUTTONDOWN,"SetUserClickedTrue")
...
; WEnd
; $hWndGUI, $MsgID, $WParam, $LParam

Func SetUserClicked($Value)
	If $Value Then
		$DidUserClick = True
	Else
		$DidUserClick = False
...
Func SetUserClickedTrue()
	$DidUserClick = True
EndFunc

Func UserRightClicked()
	; $hWndGUI, $MsgID, $WParam, $LParam
	; GUIRegisterMsg($WM_AUTOITRBUTTONDOWN,"")
	$msg = GUIGetMsg()
	;$n += 1
    ;If $n > 25 Then
     ;   $n = 25
